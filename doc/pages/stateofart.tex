...
\item	\textbf{Stemming}
	...
	Stemming is a process of getting word steam from its inflected and derived forms. The main algorithm criteria is the fact that the steam has to be identical to the morphological root. There are several types of stemming algorithms. Each of them differs in respect to accuracy and achieved performance.
...
\item	 \textbf{Lemmatisation}
	 ...
	 The lemmatisation algorithm by using vocabulary and morphological analysis of words, reduces  inflectional forms of a word to a base/root form (or dictionary form known as \textit{lemma}). This process is similar, but not identical to stemming \cite{lemma}. 
...
\subsection{Data mining - Documents clusterization}
	...
	This section focuses on important classification algorithm called K-Nearest Neighbors (kNN). It is simple and efficient to use, as it does not require any labeling for data processing \cite{knn}.
